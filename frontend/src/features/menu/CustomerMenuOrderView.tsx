...
import { Search, Plus, Filter, ShoppingCart, Minus, Tag } from 'lucide-react';
    ...
    setCart(prev => {
      return prev.map(i => {
        if (i.id === id) {
          ...
          return newQuantity > 0 ? { ...i, cartQuantity: newQuantity } : i;
        }
        return i;
        ...
        <div className="page-header" style={{ marginBottom: 'var(--sp-4)' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            <Input
              ...
              icon={<Search size={16} />}
              style={{ width: 240, paddingLeft: '36px' }}
            />
              ...
              <Card variant="elevated" style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                <Card.Content style={{ flex: 1, padding: 'var(--sp-4)', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16 }}>
                    <span style={{ fontSize: 20, fontWeight: 800, color: 'var(--orange-600)' }}>
                      ${typeof item.price === 'number' ? item.price.toFixed(2) : item.price}
                    </span>
                    <Button variant="outline" size="small" onClick={() => addToCart(item)}>
                      <Plus size={16} /> Add
                    </Button>
                  </div>
                </Card.Content>
                    ...
                    {cart.map(c => (
                      <div key={c.id} style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                        <div style={{ width: 60, height: 60, borderRadius: 8, overflow: 'hidden' }}>
                          <img
                            src={c.imageUrl || 'https://images.unsplash.com/photo-1546964124-0cce460f38ef?w=400&h=300&fit=crop'}
                            alt={c.name}
                            style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                          />
                        </div>
