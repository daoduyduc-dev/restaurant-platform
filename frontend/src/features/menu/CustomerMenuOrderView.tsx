...
export const CustomerMenuOrderView = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (!user) {
        setTimeout(() => {
          toast.success(t('menu.thankYou'));
          navigate('/');
        }, 1500);
      }
    } catch (error: any) {
      const message = error?.response?.data?.message || t('menu.placeOrderError');
      toast.error(message);
    }
  };
    ...
    <div style={{ display: 'flex', height: '100%', gap: 'var(--sp-6)' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div className="page-header" style={{ marginBottom: 'var(--sp-4)' }}>
          <div>
            <h1 style={{ color: 'var(--orange-600)' }}>{t('menu.chooseForYourTable')}</h1>
            <p>
              {canOrder
                ? t('menu.orderPrompt', { tableLabel })
                : t('menu.completeReservationFirst')}
            </p>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            ...
            <Button
              ...
              disabled={!canOrder}
            >
              <ShoppingCart size={16} /> {t('menu.cart', { count: cart.length })}
            </Button>
          </div>
        </div>
              ...
              <Card variant="elevated" style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                ...
                <Card.Content style={{ flex: 1, padding: 'var(--sp-4)', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 16 }}>
                    <span style={{ fontSize: 20, fontWeight: 800, color: 'var(--orange-600)' }}>{formatVndCurrency(item.price, i18n.language)}</span>
            ...
            <Card variant="elevated" style={{ height: '100%', display: 'flex', flexDirection: 'column', borderTop: '4px solid var(--orange-500)' }}>
              <Card.Header style={{ borderBottom: '1px solid var(--border-main)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <Card.Title>{t('menu.orderSummary')}</Card.Title>
                  <button
                    ...
                    {cart.map((cartItem) => (
                      <div key={cartItem.id} style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                        <div style={{ width: 60, height: 60, borderRadius: 8, overflow: 'hidden' }}>
                          <img
                            src={resolveMediaUrl(cartItem.imageUrl, DEFAULT_FALLBACK_IMAGE_URL)}
                            alt={cartItem.name}
                            style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                            onError={(e) => {
                              const img = e.currentTarget;
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ color: 'var(--orange-600)', fontWeight: 700 }}>
                            {formatVndCurrency(cartItem.price * cartItem.cartQuantity, i18n.language)}
                          </div>
                        </div>
              ...
              <Card.Footer style={{ padding: 'var(--sp-4)', background: 'var(--gray-50)', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', fontWeight: 700, fontSize: 18 }}>
                  <span>Tạm tính</span>
                  <span>{formatVndCurrency(cartTotal, i18n.language)}</span>
                </div>
                <Button
                  variant="primary"
